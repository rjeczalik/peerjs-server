  ...
  socket.on('message', function(data) {
    try {
      ...
      if (message.type === "PING") {
        try {
          // ignore payload for performance reasons
          socket.send(JSON.stringify({ type: 'PONG' }));
        } catch (e) {
          util.prettyError('Error responding to ' + id + ' ping: ' + e.toString());
        }
      } else if (message.type === "PONG") {
        // ignore PONG messages
      } else if (['LEAVE', 'CANDIDATE', 'OFFER', 'ANSWER'].indexOf(message.type) !== -1) {
